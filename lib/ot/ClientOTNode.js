...
class ClientOTNode {
  ...
  fetch(currentCommitId) {
    ...
    return (0, _asyncToGenerator3.default)(function* () {
      return _this4._fetchRequest(currentCommitId, false);
    })();
  }
  ...
  poll(currentCommitId) {
    ...
    return (0, _asyncToGenerator3.default)(function* () {
      return _this5._fetchRequest(currentCommitId, true);
    })();
  }

  _fetchRequest(currentCommitId, usePolling) {
    var _this6 = this;

    return (0, _asyncToGenerator3.default)(function* () {
      const serializedKey = _this6._keySerializer.serialize(currentCommitId);
      const response = yield _this6._fetch(_path2.default.join(_this6._url, usePolling ? 'poll' : 'fetch') + '?id=' + encodeURIComponent((0, _stringify2.default)(serializedKey)));
      const json = yield response.json();
      return {
        revision: _this6._keySerializer.deserialize(json.id),
        level: json.level,
        diffs: json.diffs.map(function (diff) {
          return _this6._diffSerializer.deserialize(diff);
        })
      };
    })();
  }
